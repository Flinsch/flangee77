#ifndef F77_TESTS_CL7_TEXT_METRICS_H
#define F77_TESTS_CL7_TEXT_METRICS_H
...
TESTLABS_CASE( u8"CoreLabs:  text:  metrics:  Levenshtein distance" )
{
    struct Entry
    {
        cl7::u8string s1;
    ...
    TESTLABS_SUBCASE_BATCH_WITH_DATA_STRING( u8"", container, entry, entry.s1 + u8" <-> " + entry.s2 )
    {
        ...
        TESTLABS_CHECK_EQ( cl7::text::metrics::levenshtein( s1, s2 ), expected_distance );
        TESTLABS_CHECK_EQ( cl7::text::metrics::levenshtein_normalized( s1, s2 ), expected_normalized );
    }
}



#endif // F77_TESTS_CL7_TEXT_METRICS_H
