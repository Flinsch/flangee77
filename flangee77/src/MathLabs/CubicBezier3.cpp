...
namespace ml7 {
     ...
     * Swap operation.
     */
    void CubicBezier3::swap(CubicBezier3& other) noexcept
    {
        p0.swap(other.p0);
        p1.swap(other.p1);
        p2.swap(other.p2);
     ...
     * than 1.
     */
    Vector3 CubicBezier3::point_at(float t) const
    {
     ...
     * than 1.
     */
    Vector3 CubicBezier3::tangent_at(float t) const
    {
        const float u = 1.0f - t;
     ...
     * than 1.
     */
    Vector3 CubicBezier3::second_derivative_at(float t) const
    {
        const float u = 1.0f - t;
     ...
     */
    float CubicBezier3::parameter(const Vector3& point) const
    {
        return distance3::point_cubic_bezier(point, *this).t;
    }

    /**
     * Finds the closest point on this Bézier curve to the specified query point.
     */
    Vector3 CubicBezier3::closest_point(const Vector3& point) const
    {
        return distance3::point_cubic_bezier(point, *this).point;
    }
     ...
     * curve.
     */
    float CubicBezier3::distance(const Vector3& point) const
    {
        return distance3::point_cubic_bezier(point, *this).distance();
    }
     ...
     * this Bézier curve.
     */
    float CubicBezier3::distance_squared(const Vector3& point) const
    {
        return distance3::point_cubic_bezier(point, *this).distance_squared;
    }
     ...
     * than 1.
     */
    std::pair<CubicBezier3, CubicBezier3> CubicBezier3::subdivide(float t) const
    {
        const float u = 1.0f - t;

        const Vector3 p01 = u * p0 + t * p1;
        const Vector3 p12 = u * p1 + t * p2;
        const Vector3 p23 = u * p2 + t * p3;
        const Vector3 p012 = u * p01 + t * p12;
        const Vector3 p123 = u * p12 + t * p23;
        const Vector3 p0123 = u * p012 + t * p123;
