 ...
 *
 * Tags may be nested to arbitrary depth; the effective style at any point is the
 * merge of all currently-open tags on top of `base_style`'s (inherited) GlyphStyle,
 * e.g., `[b][color=#ff0000]bold red[/color] just bold[/b]` works as expected.
 * `base_style` is only used for this seeding, it is not otherwise modified or
 * consulted.
 *
 * The grammar is deliberately strict and closed: anything that doesn't exactly
 * match one of the four tags above: an unknown tag name, a malformed/missing
 * attribute, a stray or mismatched closing tag, or an unresolved icon name, these
 * are left in the output as literal text, unchanged (including their brackets).
 * This means arbitrary game-UI text containing bracketed key hints (e.g.,
 * "[E] Interact") passes through untouched, with no escaping mechanism needed.
 *
