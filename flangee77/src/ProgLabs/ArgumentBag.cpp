...
namespace pl7 {
        ...
        : ArgumentBag()
    {
        ...
        {
            _arguments.emplace_back(cl7::text::codec::reinterpret_utf8(argv[i]));
        ...
        : ArgumentBag()
    {
        for (int i = 0; i < argc; ++i)
        {
            _arguments.emplace_back(cl7::text::codec::to_utf8(cl7::text::codec::reinterpret_utfx(argv[i])));
            _try_resolve_quotation(_arguments.back());
        }
    ...
    void ArgumentBag::_try_resolve_quotation(cl7::u8string& option_value)
    {
        ...
        if (option_value.front() == u8'"' && option_value.back() == u8'"')
        {
            std::regex regex{R"(^"[^"\\]*(?:\\.[^"\\]*)*"$)"};
            std::cmatch m;
            std::string s{cl7::text::codec::reinterpret_utf8(option_value)};
