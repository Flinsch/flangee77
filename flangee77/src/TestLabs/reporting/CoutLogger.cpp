...
namespace tl7::reporting {
    ...
    static Tcout& operator<<(Tcout& cout, cl7::u8string_view u8s)
    {
        if constexpr (std::is_same_v<typename std::remove_reference_t<decltype(cout)>::char_type, wchar_t>)
            ...
            cout << cl7::text::codec::reinterpret_utf8(u8s);
        return cout;
    }
    ...
    static cl7::u8string_view _file_path(const char* file_path)
    {
        ...
        if (sep == cl7::u8string::npos)
            return cl7::text::codec::reinterpret_utf8(file_path);
    ...
    void CoutLogger::_log_final_stats(const Stats& stats) const
    {
        ...
        auto write_group = [&cout](cl7::u8string_view label, const Stats::Group& group) {
            cout << cl7::text::transform::padded_right(cl7::u8string(label), 11, u8' ') << ColorCode::Default;
            cout << cl7::text::transform::padded_left(cl7::to_string(group.total_count), 11, u8' ') << " total" << ColorCode::Default;
            cout << ColorCode::Success << cl7::text::transform::padded_left(cl7::to_string(group.pass_count), 11, u8' ') << " passed" << ColorCode::Default;
            if (group.fail_count > 0)
                cout << ColorCode::Error << cl7::text::transform::padded_left(cl7::to_string(group.fail_count), 11, u8' ') << " failed" << ColorCode::Default;
            cout << '\n';
        };

        auto write_single = [&cout](cl7::u8string_view label, unsigned count, ColorCode color_code, cl7::u8string_view suffix) {
            cout << cl7::text::transform::padded_right(cl7::u8string(label), 11, u8' ') << ColorCode::Default;
            cout << color_code << cl7::text::transform::padded_left(cl7::to_string(count), 11, u8' ') << suffix << ColorCode::Default;
            cout << '\n';
        };
        ...
        if (hours > 0)
            ...
            cout << seconds << '.' << cl7::text::transform::padded_left(cl7::to_string(milliseconds), 3, '0') << " seconds";
        else
