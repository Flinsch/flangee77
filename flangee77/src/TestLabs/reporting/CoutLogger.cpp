...
namespace reporting {
    ...
    cl7::string_view _file_path(const cl7::char_type* file_path)
    {
    ...
    void CoutLogger::on_end_case(const Stats& stats)
    {
        ...
        else if ( stats.assertions.fail_count > 0 )
            std::cout << ColorCode::Error << 'E' << ColorCode::Default;
    ...
    {
        _current_meta = _logged_meta = nullptr;
        for ( const auto& entry : _buffered_results )
        {
            _current_meta = &entry.first;
            _log_results( entry.second );
        }
        _buffered_results.clear();
    }
     ...
     */
    void CoutLogger::_log_final_stats(const Stats& stats)
    {
#ifdef UNICODE
        auto& cout = std::wcout;
#else
