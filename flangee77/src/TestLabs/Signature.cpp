...
namespace tl7 {
    ...
    {
        assert(function_name == rhs.function_name || std::string_view(function_name) == std::string_view(rhs.function_name));
    ...
    size_t Signature::hash::operator()(const Signature& sig) const
    {
        ...
        const size_t hi = sig.line_number;
        const size_t lo = sig.iteration_number < 0 ? m : sig.iteration_number;
        assert(hi < m);
        assert(lo <= m);
        return (hi << s) | lo;
    }
