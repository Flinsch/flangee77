...
#define TL7_INTERNAL_TRY_POST_RESULT_1(expression, make_result, macro_base) \
    do { \
        ...
        ctx.try_post_result(tl7::ResultBuilder().make_result(ctx, (u8##macro_base "( " #expression " )"), (u8##macro_base "( ") + tl7::internals::to_string(!!(eval)) + u8" )", __FILE__, __LINE__, tl7::Result::make_outcome(eval))); \
    } while(false)
...
#define TL7_INTERNAL_TRY_POST_RESULT_2(expr1, expr2, make_result, macro_base, op) \
    do { \
        ...
        ctx.try_post_result(tl7::ResultBuilder().make_result(ctx, (u8##macro_base "( " #expr1 " " #op " " #expr2 " )"), (u8##macro_base "( ") + tl7::internals::to_string(eval1) + (u8" " #op " ") + tl7::internals::to_string(eval2) + u8" )", __FILE__, __LINE__, tl7::Result::make_outcome(eval1 op eval2))); \
    } while(false)
...
#define TL7_INTERNAL_TRY_POST_RESULT_EQ_STR(expr1, expr2, make_result, macro_base, op) \
    do { \
        const auto eval1 = expr1; \
        const auto eval2 = expr2; \
        ctx.try_post_result(tl7::ResultBuilder().make_result(ctx, (u8##macro_base "( " #expr1 " " #op " " #expr2 " )"), (u8##macro_base "( ") + tl7::internals::to_string(eval1) + (u8" " #op " ") + tl7::internals::to_string(eval2) + u8" )", __FILE__, __LINE__, tl7::Result::make_outcome(cl7::to_string(eval1) op cl7::to_string(eval2)))); \
    } while(false)
...
#define TL7_INTERNAL_TRY_POST_RESULT_EQ_FLT(expr1, expr2, make_result, macro_base, op) \
    do { \
        const auto eval1 = expr1; \
        const auto eval2 = expr2; \
        ctx.try_post_result(tl7::ResultBuilder().make_result(ctx, (u8##macro_base "( " #expr1 " " #op " " #expr2 " )"), (u8##macro_base "( ") + tl7::internals::to_string(eval1) + (u8" " #op " ") + tl7::internals::to_string(eval2) + u8" )", __FILE__, __LINE__, tl7::Result::make_outcome(std::abs(eval1 - eval2) < 2.0f * FLT_EPSILON))); \
    } while(false)
...
#define TL7_INTERNAL_TRY_POST_RESULT_EQ_DBL(expr1, expr2, make_result, macro_base, op) \
    do { \
        const auto eval1 = expr1; \
        const auto eval2 = expr2; \
        ctx.try_post_result(tl7::ResultBuilder().make_result(ctx, (u8##macro_base "( " #expr1 " " #op " " #expr2 " )"), (u8##macro_base "( ") + tl7::internals::to_string(eval1) + (u8" " #op " ") + tl7::internals::to_string(eval2) + u8" )", __FILE__, __LINE__, tl7::Result::make_outcome(std::abs(eval1 - eval2) < 2.0 * DBL_EPSILON))); \
    } while(false)
...
#define TL7_INTERNAL_TRY_POST_RESULT_EQ_ROUND(num_decimals, expr1, expr2, make_result, macro_base, op) \
    do { \
        const auto eval1 = expr1; \
        const auto eval2 = expr2; \
        ctx.try_post_result(tl7::ResultBuilder().make_result(ctx, (u8##macro_base "( " #expr1 " " #op " " #expr2 " )"), (u8##macro_base "( ") + tl7::internals::to_string(eval1) + (u8" " #op " ") + tl7::internals::to_string(eval2) + u8" )", __FILE__, __LINE__, tl7::Result::make_outcome(ml7::round(eval1, num_decimals) op ml7::round(eval2, num_decimals)))); \
    } while(false)
