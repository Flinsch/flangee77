...
#define TL7_INTERNAL_REGISTER_GLOBAL_FUNC(func, name) \
    TL7_INTERNAL_CONSUME_REGISTRATION( PPCAT(_tl7_foo_, __COUNTER__), tl7::TestSuite::instance().register_test_case( func, name, TEXT(__FILE__), __LINE__ ) )
...
#define TL7_INTERNAL_DEFINE_AND_SWITCH_SUBCASE_BATCH_ITERATION_BRANCH(subcase, name, iteration_number, iterable_data_container, data, tmp, stringifiable) \
    ...
    for ( auto tmp = std::begin(iterable_data_container); tmp != std::end(iterable_data_container); ++tmp, ++iteration_number ) \
        if ( const tl7::SubcaseBranchSwitch& subcase = tl7::SubcaseBranchSwitch( ctx, name + cl7::string(TEXT(" [")) + tl7::internals::to_string(iteration_number) + cl7::string(TEXT("]")), TEXT(__FILE__), __LINE__, (data = *tmp, TL7_INTERNAL_TRY_SET_SUBCASE_DATA( stringifiable ), iteration_number) ) )
...
#define TL7_INTERNAL_TRY_POST_RESULT_EQ_FLT(expr1, expr2, make_result, macro_base, op) \
    ctx.try_post_result( tl7::ResultBuilder().make_result( ctx, TEXT(macro_base "( " #expr1 " " #op " " #expr2 " )"), TEXT(macro_base "( ") + tl7::internals::to_string(expr1) + TEXT(" " #op " ") + tl7::internals::to_string(expr2) + TEXT(" )"), TEXT(__FILE__), __LINE__, tl7::Result::make_outcome( ::abs((expr1) - (expr2)) < 2.0f*FLT_EPSILON ) ) )

#define TL7_INTERNAL_TRY_POST_RESULT_EQ_DBL(expr1, expr2, make_result, macro_base, op) \
    ctx.try_post_result( tl7::ResultBuilder().make_result( ctx, TEXT(macro_base "( " #expr1 " " #op " " #expr2 " )"), TEXT(macro_base "( ") + tl7::internals::to_string(expr1) + TEXT(" " #op " ") + tl7::internals::to_string(expr2) + TEXT(" )"), TEXT(__FILE__), __LINE__, tl7::Result::make_outcome( ::abs((expr1) - (expr2)) < 2.0*DBL_EPSILON ) ) )
