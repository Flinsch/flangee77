...
namespace dl7::json::util {
    ...
    bool Unescaper::unescape_string(cl7::u8osstream& out, cl7::u8string_view json_string) const
    {
        ...
        struct EncodingErrorHandler
            : public cl7::strings::DefaultErrorHandler
        {
            ...
            EncodingErrorHandler(const Unescaper::ErrorHandler* error_handler, unsigned* error_count)
                : cl7::strings::DefaultErrorHandler(error_handler->log_context)
        ...
        protected:
            void _on_any_error(cl7::strings::EncodingError error) const override
            {
                ++*_error_count;
            }

            void _on_decode_error(cl7::strings::EncodingError error, cl7::u8string_view input_read) const override
            {
                _error_handler->on_invalid_utf8_encoding(input_read);
            }

            void _on_decode_error(cl7::strings::EncodingError error, cl7::u16string_view input_read) const override
            {
        ...
        cl7::strings::Decoder<cl7::u16char_t> utf16_decoder{&encoding_error_handler};
        cl7::u16char_t utf16_buffer[2];

        cl7::strings::Decoder<cl7::u8char_t>::iterator it{json_string, &encoding_error_handler};
        ...
        for (; it != end; ++it)
        {
            ...
            case u8'u':
            {
                if (pos + 6 > json_string.size() || !cl7::strings::is_hex_digit(json_string[pos + 2]) || !cl7::strings::is_hex_digit(json_string[pos + 3]) || !cl7::strings::is_hex_digit(json_string[pos + 4]) || !cl7::strings::is_hex_digit(json_string[pos + 5]))
                {
                    // Invalid `\uXXXX` sequence.
                    size_t count = 0;
                    while (pos + count + 2 < json_string.size() && cl7::strings::is_hex_digit(json_string[pos + count + 2]))
                        ++count;
                    assert(count < 4);
                ...
                codepoint.value = cl7::strings::parse_hex(json_string.substr(pos + 2, 4));
                pos += 6;

                // Check for surrogate pair.
                if (codepoint.is_high_surrogate() && pos + 6 <= json_string.size() && json_string[pos + 0] == u8'\\' && json_string[pos + 1] == u8'u' && cl7::strings::is_hex_digit(json_string[pos + 2]) && cl7::strings::is_hex_digit(json_string[pos + 3]) && cl7::strings::is_hex_digit(json_string[pos + 4]) && cl7::strings::is_hex_digit(json_string[pos + 5]))
                {
                    auto second_codepoint = cl7::strings::codepoint{cl7::strings::parse_hex(json_string.substr(pos + 2, 4))};
            ...
            case u8'x':
            {
                if (pos + 4 > json_string.size() || !cl7::strings::is_hex_digit(json_string[pos + 2]) || !cl7::strings::is_hex_digit(json_string[pos + 3]))
                {
                ...
                dummy_oss << cl7::strings::parse_hex<cl7::u8char_t>(json_string.substr(pos + 2, 2));
                pos += 4;

                // Look for more of those `\xXX` and concatenate them all
                // to capture any UTF-8 sequences encoded in this way.
                while (pos + 4 <= json_string.size() && json_string[pos + 0] == u8'\\' && json_string[pos + 1] == u8'x' && cl7::strings::is_hex_digit(json_string[pos + 2]) && cl7::strings::is_hex_digit(json_string[pos + 3]))
                {
                    dummy_oss << cl7::strings::parse_hex<cl7::u8char_t>(json_string.substr(pos + 2, 2));
                    pos += 4;
                }
                ...
                cl7::strings::Decoder<cl7::u8char_t>::iterator dummy_it{dummy_string, &encoding_error_handler};
