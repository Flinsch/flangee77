...
namespace dl7::json::detail {
    ...
    {
        diagnostics->add(syntax::Diagnostic::Severity::Warning, u8"Invalid UTF-8 encoding.", token.source_location);
    }

    void UnescaperErrorHandler::on_invalid_utf16_encoding(cl7::u16string_view sequence) const
    {
        diagnostics->add(syntax::Diagnostic::Severity::Warning, u8"Invalid UTF-16 encoding.", token.source_location);
    }
