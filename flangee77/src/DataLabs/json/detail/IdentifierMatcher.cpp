...
namespace dl7::json::detail {
    ...
    size_t IdentifierMatcher::operator()(cl7::u8string_view source) const
    {
        ...
        if (!cl7::text::inspect::is_ascii_letter(source[0]) && source[0] != u8'_' && source[0] != u8'$')
            return 0;

        // Remaining characters can be letters, digits, underscores, or dollar signs.
        size_t i = 1;
        while (i < source.size() && (cl7::text::inspect::is_ascii_letter(source[i]) || cl7::text::inspect::is_digit(source[i]) || source[i] == u8'_' || source[i] == u8'$'))
            ++i;
