...
namespace dl7::ini::detail {
    ...
    string_t Builder::_parse_section_name(syntax::TokenReader& token_reader)
    {
        ...
        return cl7::strings::trimmed(oss.str());
    }
    ...
    string_t Builder::_parse_unquoted_string(syntax::TokenReader& token_reader)
    {
        ...
        while (token_reader.is_valid() && !is_eol)
        {
            ...
            {
                auto tmp = cl7::strings::trimmed(oss.str());
                oss = cl7::u8osstream();
                oss << tmp;
                oss << u8" ";
                token_reader.next_token();
            ...
            default:
                for (cl7::strings::Decoder<cl7::u8char_t>::iterator it{token_reader.consume_token().lexeme, &encoding_error_handler};
                    it != cl7::strings::Decoder<cl7::u8char_t>::sentinel{};
                    ++it)
                {
