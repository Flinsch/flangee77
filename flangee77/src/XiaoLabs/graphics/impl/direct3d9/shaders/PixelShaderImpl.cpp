...
namespace xl7::graphics::impl::direct3d9::shaders {
     ...
     */
    bool PixelShaderImpl::_acquire_precompiled_impl(const graphics::shaders::CodeDataProvider& code_data_provider)
    {
        ...
        if (FAILED(hresult))
        {
            LOG_ERROR(errors::d3d9_result(hresult, u8"IDirect3DDevice9::CreatePixelShader"));
            return false;
        }
     ...
     * it contains additional implementation-specific information.
     */
    bool PixelShaderImpl::_acquire_recompilable_impl(const graphics::shaders::CodeDataProvider& code_data_provider, graphics::shaders::ShaderCode& bytecode_out)
    {
        const cl7::Version& version = GraphicsSystem::instance().get_rendering_device()->get_capabilities().shaders.pixel_shader_version;
        const cl7::astring target = "ps_" + std::to_string(version.major) + "_" + std::to_string(version.minor);
     ...
     */
    bool PixelShaderImpl::_recompile_impl(const graphics::shaders::CompileOptions& compile_options, graphics::shaders::ShaderCode& bytecode_out)
    {
     ...
     */
    bool PixelShaderImpl::_reflect_impl(const graphics::shaders::ShaderCode& bytecode, graphics::shaders::ReflectionResult& reflection_result_out)
    {
