...
namespace xl7::graphics::textures {
        ...
        , _desc(desc)
    {
        ...
        if (capabilities.textures.max_cubemap_size && _desc.width > capabilities.textures.max_cubemap_size)
            LOG_WARNING(u8"The " + get_qualified_identifier() + u8" to be created has a width of " + cl7::to_string(_desc.width) + u8", but a maximum of " + cl7::to_string(capabilities.textures.max_cubemap_size) + u8" is supported.");
        if (capabilities.textures.max_cubemap_size && _desc.height > capabilities.textures.max_cubemap_size)
            LOG_WARNING(u8"The " + get_qualified_identifier() + u8" to be created has a height of " + cl7::to_string(_desc.height) + u8", but a maximum of " + cl7::to_string(capabilities.textures.max_cubemap_size) + u8" is supported.");
